...
## מיגרציה 013 – קבלה על שם אחר (F-76) ומצב ניהול השם (F-13)

### `vow_payment.receipt_name` / `donation.receipt_name`

`NULL` = הקבלה על שם החבר או התורם, כפי שהיה עד היום. ערך = הקבלה תצא על
שם אחר: חברה בע"מ, עמותה או בן משפחה.

העמודה יושבת על **התשלום ועל התרומה** ולא רק על הקבלה. הקבלה מקפיאה את
`payer_name` בהפקה (B-04), וכשמבטלים ומפיקים מחדש צריך לדעת שוב על שם מי –
בלי זה כל הפקה חוזרת הייתה חוזרת לשם החבר בשקט.

הנפילה חזרה לשם הרגיל היא על מחרוזת ריקה ולא רק על `NULL`: שדה שנפתח
ונשאר ריק אינו בקשה לקבלה בלי שם, וקבלה בלי שם משלם היא מסמך פסול.

### `setting.member_name_mode`

`split` (ברירת מחדל) = שם פרטי ושם משפחה בנפרד. `full` = שדה אחד.

**אין שינוי סכימה.** במצב `full` הערך נשמר ב-`first_name` ו-`last_name`
נשאר ריק, ולכן מעבר בין המצבים אינו מאבד דבר ואינו דורש המרה: בית כנסת
שניהל שם מלא עובר ל-`split` וממלא את שם המשפחה חבר-חבר.

האימות בשרת (`assertNameFilled`) ולא רק בטופס: ה-renderer אינו הגבול,
וייבוא או קריאת IPC ישירה חייבים להיתקל באותו כלל.

## מיגרציה 012 – רשימת הנדרים למכירה (F-140..F-143)

הרקע: עד היום הזנת נדר ביקשה "פירוט" כטקסט חופשי. הגבאי הקליד "עליית שלישי"
בשבת אחת ו"שלישי" בשבת הבאה, ואי אפשר היה לשאול **כמה הכניסה עליית שלישי
השנה** – השאלה הבסיסית ביותר על פנקס הגבאי.

### `vow_item` – קטלוג הכיבודים

| עמודה | טיפוס | הערות |
|---|---|---|
| `id` | INTEGER PK | |
| `name` | TEXT NOT NULL UNIQUE | שם המצווה / הכיבוד |
| `category` | TEXT | קטגוריית הגיליון בפנקס. טקסט חופשי – בית כנסת אחר יחלק אחרת |
| `duration` | TEXT | משך המצווה: "חד-פעמי (אותה תפילה)", "שנתי" |
| `sale_timing` | TEXT | **המועד הקובע** – מתי הכיבוד נמכר |
| `performance_timing` | TEXT | מתי הוא מבוצע / מי מברך |
| `scope` | TEXT NOT NULL | `shabbat` / `occasion` / `always` |
| `notes` | TEXT | |
| `sort_order` | INTEGER | |
| `is_active` | INTEGER | כיבוי במקום מחיקה, כשהכיבוד כבר נמכר |

### `vow_item_occasion` – שיוך רבים-לרבים

`(vow_item_id, occasion_id)` כמפתח. נדרש כי "חול המועד פסח" שייך גם לשבת
חול המועד פסח וגם לפסח.

### `vow_charge.vow_item_id`

מפתח זר ל-`vow_item`, `NULL` מותר. חיובים שנוצרו לפני המיגרציה אינם
מקושרים, וגם בעתיד אפשר להזין נדר בלי לבחור מהרשימה.

### הכלל שקובע את השיוך

**תזמון המכירה, לא תזמון הביצוע.** "פרנס חצי-שנתי (קיץ)" מבורך בכל שבת
במשך חצי שנה, אבל הוא נמכר בערב פסח – ולכן הוא מופיע ברשימה של פסח ולא
בכל שבת. ההחלטה יושבת ב-`vowItemScope.ts` כפונקציה טהורה ונבדקת על כל
103 השורות של הפנקס.

`scope = 'shabbat'` נכלל בשאילתה **רק כשהמועד הוא `parasha`**: ביום כיפור
אין קריאה של שבת רגילה, והצגת 18 עליות השבת שם הייתה מטביעה את מה שבאמת
נמכר.

### הזרעה – פעם אחת בלבד

בשונה משאר ה-seed, 103 הכיבודים נכתבים פעם אחת ומסומנים ב-
`setting.vow_items_seeded_at`. `INSERT OR IGNORE` בכל הפעלה היה מחזיר כל
כיבוד שהגבאי מחק, והרשימה הזו **נועדה להיערך**.
